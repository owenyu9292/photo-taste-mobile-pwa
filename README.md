...
휴대폰에서 사진을 보며 취향 태그, 좋음/애매/별로/다시보기, 이유 태그, 메모를 기록하고 `phone_review_patch` JSON으로 내보내는 모바일 PWA입니다.
...
- 이유 태그와 직접 태그를 남긴다.
- 메모를 남긴다.
- `photo-taste-phone-patch-*.json` 파일을 만들고 Google Drive로 전달한다.
- 노트북 메인 앱에서 이 JSON을 가져와 로컬 기준 저장소에 병합한다.
